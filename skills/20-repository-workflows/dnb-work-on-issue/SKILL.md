...
This skill may also be called by `~/.ai/skills/20-repository-workflows/dnb-work-on-next-issue/SKILL.md` after issue selection.
...
If no issue number is present, do not select an issue automatically. Ask for the issue number or use `~/.ai/skills/20-repository-workflows/dnb-select-next-issue/SKILL.md` only when the user explicitly asked to work on an unspecified open issue.
