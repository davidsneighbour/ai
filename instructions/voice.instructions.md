...
These are mechanical, low-judgment copy rules. Apply them to any prose you
write or edit, regardless of whether a dedicated writing task is in progress.
For the deeper editorial work of matching Patrick's voice (tone, structure,
first person, form-specific calibration), use the `dnb-voice` skill instead;
these rules are the baseline it builds on.
